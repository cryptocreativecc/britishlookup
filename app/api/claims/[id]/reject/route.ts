...
import { ClaimRejected } from "@/emails/claim-rejected";
...
export async function POST(
  ...
  try {
    ...
    await adminPb.collection("claims").update(id, { status: "rejected" });
    ...
    if (claimUser?.email && business) {
      try {
        await sendEmail({
          to: claimUser.email,
          subject: `Claim Update: ${business.name}`,
          react: createElement(ClaimRejected, {
            name: claimUser.name,
    ...
    console.error("Claim reject error:", e);
